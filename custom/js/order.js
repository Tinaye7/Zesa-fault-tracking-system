...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			var clientVehicle = $("#clientVehicle").val();
			var clientPlate = $("#clientPlate").val();
			var clientAmount = $("#clientAmount").val();
			var color = $("#color").val();
			...
			if(clientVehicle == "") {
				$("#clientVehicle").after('<p class="text-danger"> The Vehicle Name field is required </p>');
				$('#clientVehicle').closest('.form-group').addClass('has-error');
			} else {
				$('#clientVehicle').closest('.form-group').addClass('has-success');
			} // /else
			if(clientPlate == "") {
				$("#clientPlate").after('<p class="text-danger"> The Contact field is required </p>');
				$('#clientPlate').closest('.form-group').addClass('has-error');
			} else {
				$('#clientPlate').closest('.form-group').addClass('has-success');
			} // /else
			if(clientAmount == "") {
				$("#clientAmount").after('<p class="text-danger"> The amount paid field is required </p>');
				$('#clientAmount').closest('.form-group').addClass('has-error');
			} else {
				$('#clientAmount').closest('.form-group').addClass('has-success');
			} // /else
			
			if(color == "") {
				$("#color").after('<p class="text-danger"> The color code field is required </p>');
			...
			if(volume == "") {
				$("#volume").after('<p class="text-danger"> The volume used field is required </p>');
				$('#volume').closest('.form-group').addClass('has-error');
			} else {
				$('#volume').closest('.form-group').addClass('has-success');
			} // /else

			// array validation
			var productCode = document.getElementsByName('productCode[]');				
			var validateProduct;
			for (var x = 0; x < productCode.length; x++) {       			
				var productCodeId = productCode[x].id;	    	
		    if(productCode[x].value == ''){	    		    	
		    	$("#"+productCodeId+"").after('<p class="text-danger"> Product Code Field is required!! </p>');
		    	$("#"+productCodeId+"").closest('.form-group').addClass('has-error');	    		    	    	
	      } else {      	
		    	$("#"+productCodeId+"").closest('.form-group').addClass('has-success');	    		    		    	
	      }          
	   	} // for
		   ...
		   var productCode = document.getElementsByName('productName[]');				
			var validateName;
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && color&&volume&&clientContact && clientVehicle&&clientPlate&& clientAmount) {
				if(validateProduct == true && validateQuantity == true && validateName == true) {
					// create order button
		...
		$("#editOrderForm").unbind('submit').bind('submit', function() {
			...
			if(clientPlate == "") {
				$("#clientPlate").after('<p class="text-danger"> The plate number field is required </p>');
				$('#clientPlate').closest('.form-group').addClass('has-error');
			} else {
				$('#clientPlate').closest('.form-group').addClass('has-success');
			} // /else
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact &&color&&volume&& clientVehicle && clientPlate && clientAmount) {
				if(validateProduct == true && validateQuantity == true && validateName== true) {
					$("#createOrderBtn").button('loading');
	...
	if(orderId) {		
		...
		$.ajax({
			...
			success:function(response) {
				...
				var mywindow = window.open('', 'Highgrade Motors', 'height=400,width=600');
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				'<td>'+
					...
					'<select class="form-control" name="productCode[]" id="productCode'+count+'" onchange="getProductData('+count+')" >'+
				...
				'<td style="padding-left:20px;"">'+
					'<input type="text" name="productName[]" id="productName'+count+'" autocomplete="off" disabled="true" class="form-control" />'+
					'<input type="hidden" name="productName[]" id="productName'+count+'" autocomplete="off" class="form-control" />'+
				'</td style="padding-left:20px;">'+
				'<td style="padding-left:20px;">'+
					'<div class="form-group">'+
				...
				'<td style="padding-left:20px;">'+
					'<div class="form-group">'+
					'<input type="float" name="quantity[]" id="quantity'+count+'" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control" min="1" />'+
					'</div>'+
				'</td>'+
...
function getProductData(row = null) {
	...
	if(row) {
		var productId = $("#productCode"+row).val();		
	...
	if(orderId) {
		...
		$.ajax({
			...
			success:function(response) {				
				...
				var iquantity = response.order[9] ;
